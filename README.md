...
| `MATCHING_COMPATIBILITY_THRESHOLD` | минимальная семантическая близость для ребра | `0.50` |
| `ANALYSIS_CATEGORY_SIMILARITY_THRESHOLD` | минимальная уверенность автокатегоризации | `0.65` |
| `ANALYSIS_CATEGORY_CONFIDENCE_MARGIN` | минимальный отрыв первой категории от второй | `0.05` |
| `ANALYSIS_RECOVERY_POLL_INTERVAL` | период поиска зависшего анализа | `30s` |
| `ANALYSIS_STALE_AFTER` | возраст записи `ANALYZING` для повторной обработки | `5m` |
| `ANALYSIS_RECOVERY_BATCH_SIZE` | максимум записей за проход recovery-worker | `100` |
...
Миграция `000005` добавляет категории предложения и желания, оценки качества
описания и изображения, долю успешных обменов пользователя и время последней
смены статуса. После создания карточка проходит асинхронный анализ и переходит
из `ANALYZING` в `MATCHING`; recovery-worker повторно обрабатывает зависшие
карточки. Если категории ещё не наполнены, карточка помечается для ручной
категоризации, но семантический matching продолжает работать по векторам.
