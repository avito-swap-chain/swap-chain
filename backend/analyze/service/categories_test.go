...
func TestCategoryBootstrapFillsMissingEmbeddings(t *testing.T) {
	repo := &categoryBootstrapRepo{
		missing: []db.ListCategoriesMissingEmbeddingRow{
...
func TestCategoryBootstrapRejectsWrongEmbeddingDimensions(t *testing.T) {
	repo := &categoryBootstrapRepo{
		missing: []db.ListCategoriesMissingEmbeddingRow{{ID: 1, Name: "Электроника"}},
		total:   1,
	}
...
func (repo *categoryBootstrapRepo) ListCategoriesMissingEmbedding(context.Context) ([]db.ListCategoriesMissingEmbeddingRow, error) {
	if repo.err != nil {
		return nil, repo.err
	}
...
func (repo *categoryBootstrapRepo) SetCategoryEmbedding(_ context.Context, arg db.SetCategoryEmbeddingParams) (int64, error) {
	if repo.err != nil {
		return 0, repo.err
	}
	repo.updated = append(repo.updated, arg.ID)
	return 1, nil
}
